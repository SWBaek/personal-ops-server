...
- On Windows PowerShell, never pipe a here-string or ordinary PowerShell string containing non-ASCII text directly to a native CLI with the default output encoding. In particular, do not use `$body | gh ... --body-file -` unless `$OutputEncoding` and the console output encoding have first been set explicitly to BOM-less UTF-8 for that process. The default native-pipeline encoding can irreversibly replace Korean text with `?`.
- Prefer an explicitly UTF-8 temporary body file outside the repository with `gh ... --body-file <path>` for multiline GitHub issue, pull-request, and comment bodies. Do not keep private or temporary GitHub payloads in the worktree. If a direct `--body` argument is used, account for Windows command-line length and embedded-quote handling.
- After every GitHub write containing non-ASCII text, read the created or edited object back with `gh` and compare the remote body with the intended text before reporting success. A successful exit code or returned URL does not prove encoding integrity. If corruption is found, edit the same object rather than posting a duplicate, switch to a verified UTF-8 transport, and verify again.
