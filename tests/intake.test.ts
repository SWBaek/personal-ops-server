...
import { parseAssistantTurnEnvelope } from "../src/domain/intake.js";
...
test("structured assistant output rejects invalid memo payloads", () => {
  ...
  assert.throws(() => parseAssistantTurnEnvelope(JSON.stringify({
    reply: "확인했습니다.",
    resolutions: [],
    memoProposal: { operation: "revise", targetMemoId: null, supersedesProposalId: null, memo: firstDraft },
...
function completion(envelope: Parameters<OpsStore["completeAssistantTurn"]>[1]["envelope"]): Parameters<OpsStore["completeAssistantTurn"]>[1] {
  return {
    envelope,
